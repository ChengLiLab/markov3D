load_raw_matrix <- function(raw_matrix_path, chr_size, resolution) {
...
mat_to_htcobj <- function(contact_matrix, chr, chr_size, resolution) {
  ...
  gr_obj <- GenomicRanges::GRanges(
    seqnames = chr,
    ranges = IRanges::IRanges(seq(0, chr_size, resolution),
      seq(0, chr_size, resolution) + resolution),
    name = seq(0, chr_size, resolution)
  )
  htc_mat <- Matrix::Matrix(as.matrix(contact_matrix))
